...
function randomColor() {
    ...
    // สีของปุ่ม
    var button = document.getElementById("button-เปลี่ยนสี");
...
// ดำเนินการเมื่อผู้เล่นกดปุ่มรีสตาร์ท
// ถามควรเป็นจริงหากเราควรถามผู้ใช้ว่าต้องการเล่นเป็น X หรือ O
function restartGame(ask) {
    if (moves > 0) {
        var response = confirm("คุณแน่ใจหรือไม่ว่าต้องการเริ่มต้นใหม่ ?");
        if (response === false) {
            return;
        }
    }
...
function makeComputerMove() {
    ...
    if (moves >= 3) {
        ...
        // ถ้าผู้เล่นเป็น X และเล่นตรงกลาง ให้เล่นมุมใดมุมหนึ่ง
        cell = corners[intRandom(0, 3)];
    } else if (moves === 2 && myGrid.cells[4] == player && computer == x && difficulty == 1) {
        // ถ้าผู้เล่นเป็น O และเล่นตรงกลาง ให้เอาสองมุมตรงข้ามกัน
        ...
        // ถ้าคอมพิวเตอร์เป็น X ให้เริ่มจากมุมใดมุมหนึ่งเป็นบางครั้ง
        cell = corners[intRandom(0, 3)];
    } else {
    ...
    // console.log("คอมพิวเตอร์เลือก " + id);
    document.getElementById(id).innerHTML = computerText;
    document.getElementById(id).style.cursor = "default";
    // สุ่มการหมุนเครื่องหมายบนกระดานเพื่อให้ดู
    //เหมือนเขียนด้วยลายมือ
    var rand = Math.random();
    if (rand < 0.3) {
        document.getElementById(id).style.transform = "rotate(180deg)";
    } else if (rand > 0.6) {
...
// ตรวจสอบว่าเกมจบลงแล้วและตัดสินผู้ชนะ
function checkWin() {
    winner = 0;
...
function getOptions() {
    ...
    for (var ครับ = 0; ครับ < diffs.length; ครับ++) {
        if (diffs[ครับ].checked) {
            difficulty = parseInt(diffs[ครับ].value);
            break;
            // debugger;
        }
    }
...
function endGame(who) {
    if (who == player) {
        announceWinner("ยินดีด้วย คุณชนะแล้ว ชักที!");
    } else if (who == computer) {
        announceWinner("ผมชนะ อ้ะครับ!");
    } else {
        announceWinner("มันเสมอกันนะ!");
    }
    gameOver = true;
    whoseTurn = 0;
    moves = 0;
