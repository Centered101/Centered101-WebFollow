...
document.addEventListener("DOMContentLoaded", function () {
    ...
    birthdaySound.addEventListener('timeupdate', function () {
        if (birthdayProgress) {
            birthdayProgress.value = birthdaySound.currentTime / birthdaySound.duration;
        }
    });
    ...
    function displayDaysSinceBirthday() {
        ...
        const hoursSinceBirthday = Math.floor((difference % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
        const minutesSinceBirthday = Math.floor((difference % (1000 * 60 * 60)) / (1000 * 60));
        const secondsSinceBirthday = Math.floor((difference % (1000 * 60)) / 1000);

        document.getElementById("daysSinceBirthday").innerHTML = `วันเกิดผ่านมาแล้ว: ${replaceZeroWithHeart(formatTime(daysSinceBirthday))} วัน ${replaceZeroWithHeart(formatTime(hoursSinceBirthday))} ชั่วโมง ${replaceZeroWithHeart(formatTime(minutesSinceBirthday))} นาที ${replaceZeroWithHeart(formatTime(secondsSinceBirthday))} วินาที;`;
